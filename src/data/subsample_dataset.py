...
def are_files_selected(current_file_root, img_list):
    ...
    try:
        flag = int(current_file_root) in img_list
    except:
        flag = current_file_root in img_list
    return flag
...
def main(args):
    ...
    try:
        ...
        with open(args.img_file, 'rb') as filehandle:
            img_list = pickle.load(filehandle)
            img_list = img_list[:args.n_imgs]
    ...
    if args.imagenet_tars_flag:
        data_dirs = [args.data_dir, args.imagenet_data_dir]  # both imagenet and laion datadirs
    else:
    ...
    for i in range(0 + args.worker_idx, len(tars), args.n_workers):
        if label_list is not None:
            count_records += process(tars[i], out_tar_stream, img_list, img_set=img_set,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--data_dir', type=str, default='/mnt/qb/datasets/laion400m/laion400m-data/',
                        help='dataset to subsample')
    parser.add_argument('-o', '--out_dir',
                        help='output folder')
    parser.add_argument('-i', '--img_file',
                        help='list or numpy file with image paths to select from the dataset')
    ...
    parser.add_argument('--imagenet_tars_flag', action='store_true')
    parser.add_argument('--imagenet_data_dir', type=str,
                        default='/mnt/beegfs/bethge/pmayilvahanan31/datasets/imagenet-train-tar/')
